> **⚠ STALE — needs regen (as of 2026-07-20 @ `8b9295a`).** This doc is 2026-07-15 vintage. The EKF pipeline/orchestrator maths are unchanged, but the replay data source gained a new entry point that drives this filter: `ImuReplayEngineV2.loadFromPolyline` + `GpsBlackoutWindow` (`imu_replay_engine_v2.dart`), synthesizing an EKF/IMU/GPS timeline from an arbitrary polyline with explicit tunnel windows. See `04_ekf_replay.md` and `SYSTEM_MAP.md §0`.
