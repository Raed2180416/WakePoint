...
### 1.12 Persisted telemetry sink + flush — GAP #7/#16
- **Repro:** `flutter test test/telemetry/` → 48 pass. `FileTelemetrySink` appends PII-free JSONL to disk (bounded buffer, size-capped rotation, fail-open), `flush()` is on the interface + service, and `configureDefaultSinks({dir})` registers it. (Emit sites — alarmOutcome/reliability/gpsLost — are the next wiring step; see STILL-OPEN.)
...
- **Telemetry EMIT sites (GAP #8/#14/#15).** The persisted sink exists and is proven (§1.12), but `alarmOutcome{on-time/late/missed}`, `reliability{OS-killed/Doze/backstop-fired}`, and `gpsLost` still have **zero emit sites** — the north-star "which phones fail to wake?" funnel is not yet capturing. Next: emit `alarmOutcome` at the fire decision (lead = trueArrival − fireTs), `reliability` on restart-after-kill (persisted clean-shutdown flag), `gpsLost` on inter-fix gaps. Design is settled (schema is PII-free and doubles as the DATA_STRATEGY seed).
