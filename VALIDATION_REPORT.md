...
### 1.16 Deep simulation & validation phase — at-scale, adversarial, and a P0 bug it caught
A second, much larger validation pass (6 workflows / ~55 agents + hands-on) generated **391 real-ish rides across all 19 cities / 46 lines / 9 scenarios** from the shipped dataset and ran the never-late logic to exhaustion. Results (all in `docs/research/underground/`):
- **Free-run cone never-late at scale — through the REAL Dart code.** `flutter test test/scale/reachability_scale_test.dart` → ran=391 fired=391 **never-fired=0, LATE=0**, driving the actual production `ReachabilityTracker`/`VLineTable` (not a model). An independent Python baseline + the scale-sim (1144 cells) agree: 0 late. The early-firing is **5 m median to target** (already tight); the kilometre-scale tail is entirely long-blackout / cold-start.
- **~29,461 adversarial trials / 50.5M steps** across 7 gap simulations (Monte-Carlo tail-risk, process-death→restore, wrong-train/off-route, multi-leg, wrong-place, an 11k-case fuzzer, overnight 4.2 h + clock jumps) found **exactly one real never-late defect in shipped code — now fixed (§1.17).** Every other axis: 0 violations.
- **Mutation testing: 8/8 injected never-late bugs CAUGHT, 0 survived** (`docs/research/underground/scripts/mutation_test.sh`) — the reachability tests have teeth; the green is not a coverage illusion.
- **Honest verdict on tightening the underground tail:** the stop-anchoring that shrinks it (−34 m vs +7279 m on a real 9-min blackout) can transiently dip below truth (1085 sub-truth dips in sim), so it is **DISPLAY-only** — the ALARM must fire on the provable free-run cone. Tightening the *alarm* underground is sensor-gated (needs on-device baro/mag for detector precision). See `docs/research/underground/SYNTHESIS.md` + `SCALE_SIMULATION.md`.

### 1.17 P0 + P1 never-late fixes the simulation surfaced
- **P0 — monotonic clock (backward-wall-jump late fire).** Reachability measured `t − t0` off the wall clock, so a backward jump (NTP/DST/manual) froze the cone → a reproducible **~28-minute LATE fire**. **Fixed** (`df34ebc`): reachability now runs on `AppClock.monotonicSeconds()` (Stopwatch-based), with the GPS-fix and OS-kill-snapshot timestamps mapped into the monotonic frame by their wall-clock age. **Repro:** `flutter test test/core/clock/monotonic_clock_test.dart`. Residual: full robustness across a wall-clock jump *during* an OS-kill needs `SystemClock.elapsedRealtime` persisted in the snapshot (device follow-up).
- **P1 — multi-leg mode-max.** The per-leg reach bound used only the current leg's V_LINE, so a blackout straddling a walk→metro boarding under-bounded (walk 2 m/s while the train does 28) → late (387/387 in sim). **Fixed** (`fca0fc3`): the bound uses the max V_LINE over the forward legs (single-leg metro unchanged; GATE PASS).
- **P2 — wrong-place early wakes (documented, not "fixed").** 46 rides fired at an adjacent station (all EARLY, never late). A progress-gate on the reach fire would *break* never-late (it fires on worst-case precisely because there's no progress evidence underground), so this is an inherent, documented UX cost of the guarantee, mitigated by the deviation monitor. The wrong-train never-late axis was clean (0 late).

**Green tree at every checkpoint:** `flutter analyze lib/` = **0 errors / 0 warnings** (84 pre-existing info-level lints only); full `flutter test` = all pass (1179+ at the latest checkpoint).
...
- **Express/airport V_LINE residual (from the scale sim).** Delhi Airport Express reported as "Orange Line" resolves to `defaultMps` (28 m/s = 100 km/h) but its design speed is ~135 km/h → under-bound → a late-fire risk on that one line. Not keyword-fixable ("Orange" collides with Nagpur's 90 km/h metro Orange); the robust fix is a per-line V_LINE from the shipped dataset, or an explicit `VLineTable` override for the handful of known fast lines. Same family as the Mumbai-suburban residual above.
- **Underground alarm-tightening = sensor-gated (from the deep sim).** Shrinking the long-blackout early-firing (the km-scale tail) safely requires a precision stop-detector, which on a handheld needs on-device barometer/magnetometer logging + a data-collection pass (`docs/research/underground/SYNTHESIS.md` roadmap). The stop-anchoring is scaffolded in research as DISPLAY-only; the alarm stays on the provable cone.
- **`elapsedRealtime` under Doze / across an OS-kill clock jump.** The monotonic-clock P0 fix (§1.17) uses a Stopwatch (correct for the live net); full robustness across a wall-clock jump *during* a process death needs `SystemClock.elapsedRealtime` persisted in the snapshot — a small native-channel + snapshot-schema device task.
