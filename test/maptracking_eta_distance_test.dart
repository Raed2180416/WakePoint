...
void main() {
  test('Computes ETA from directions step durations given snapped progress', () {
    ...
    final snap = SnapToRouteEngine.snap(
      point: const LatLng(37.0, -121.995),
    ...
    final etaSec = EtaUtils.etaRemainingSeconds(
      ...
      stepBoundariesMeters: boundaries,
  ...
  test(
    'Detects transfer boundary and computes time-to-switch with simple speed',
    () {
      final routePoints = <LatLng>[
        const LatLng(37.0, -122.0),
        const LatLng(37.0, -121.99),
      ...
      final snap = SnapToRouteEngine.snap(
        point: const LatLng(37.0, -121.9982),
        polyline: routePoints,
        hintIndex: null,
        searchWindow: 30,
      );
      final boundaries = TransferUtils.buildTransferBoundariesMeters(
        directions,
        metroMode: true,
      );
      expect(boundaries, isNotEmpty);
      final next = boundaries.firstWhere(
        (b) => b > snap.progressMeters,
        orElse: () => -1,
      );
