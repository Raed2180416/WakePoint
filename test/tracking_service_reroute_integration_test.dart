...
void main() {
  ...
  group('TrackingService deviation/reroute + manager integration', () {
    ...
    test(
      'Triggers sustained deviation and emits reroute decision; manager switches to nearby cached route',
      () async {
        ...
        final sub = svc.routeSwitchStream.listen(
          (e) => switches.add('${e.fromKey}->${e.toKey}'),
        );
        final reroutes = <bool>[];
        final rsub = svc.rerouteDecisionStream.listen(
          (r) => reroutes.add(r.shouldReroute),
        );
