...
void main() {
  ...
  setUp(() async {
    ...
    ApiClient.directionsBodiesHistory.clear();
    ApiClient.testDirectionsResponse = null;
    ApiClient.testDirectionsResponses.clear();
  ...
  test('Transit variant included in request and cache key', () async {
    await DirectionService().getDirections(
      12,
      77,
      12.5,
      77.5,
  ...
  test('Metro mode: selects fastest route that contains a metro leg', () async {
    ApiClient.testDirectionsResponses.add({
      'status': 'OK',
      'routes': [
        {
          'id': 'bus_fast',
    ...
    expect((routes.first as Map)['id'], 'metro_slightly_slower');
  });

  test(
    'Metro mode: falls back to time-anchored subway request when primary has no metro legs',
    () async {
      ApiClient.testDirectionsResponses.addAll([
        {
          'status': 'OK',
          'routes': [
            {
      ...
      expect(ApiClient.directionsCallCount, 2);
      expect(ApiClient.directionsBodiesHistory.length, 2);
      expect(ApiClient.directionsBodiesHistory[0]['transit_mode'], 'rail');
      expect(ApiClient.directionsBodiesHistory[1]['transit_mode'], 'subway');
      expect(
        ApiClient.directionsBodiesHistory[1].containsKey('departure_time'),
        isTrue,
      );

      final routes = directions['routes'] as List;
      expect((routes.first as Map)['id'], 'metro_after_fallback');
    },
  );
