...
/// Development server for remote demo triggers.
///
/// SECURITY: This server is ONLY available in debug mode and binds to
/// localhost only. It is completely disabled in release builds.
class DevServer {
  static HttpServer? _server;

  /// Whether the dev server can run (debug mode only)
  static bool get isEnabled => kDebugMode;

  static Future<void> start({int port = 8765}) async {
    // SECURITY: Completely disabled in release builds
    if (!isEnabled) {
      dev.log('DevServer disabled in release mode', name: 'DevServer');
      return;
    }

    if (_server != null) return;
    try {
      // SECURITY: Bind to loopback only (localhost), not 0.0.0.0
      _server = await HttpServer.bind(InternetAddress.loopbackIPv4, port);
      dev.log(
        'DevServer listening on localhost:$port (debug only)',
        name: 'DevServer',
      );
  ...
  static Future<void> stop() async {
    await _server?.close();
    _server = null;
    dev.log('DevServer stopped', name: 'DevServer');
  }
